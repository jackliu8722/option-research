...
$$ \mathcal{V} = 60{,}000 \times 0.38543 \times \sqrt{0.25} \approx 11{,}563 $$

这是"**每 1.0 波动**"（σ 变动 1.00 = 100 个百分点）的 Vega。实务按**每 1%**（÷100）报：**Vega ≈ 115.6 USD / 每 1% 波动**。

- IV 从 80% 升到 90%（+10 点）：这张 Call 约 10 × 115.6 = +\$1,156；DVOL crush −20 点：约 −\$2,313。
- **组合 vol crush**：净 𝒱 = +\$300/1%，DVOL 从 85% 跌到 65%（−20）→ Vega PnL ≈ 300 × (−20) = −\$6,000：即便方向没错、标的没动，单 IV 回落就亏 \$6,000。
- **币口径**：把美元 Vega 除以现价 $S$ 即得币计 Vega（Vega 不对 $S$ 求导，可简单除 $S$ ，见 [07 单位篇](./07-币本位希腊字母的单位与换算.md)）。
